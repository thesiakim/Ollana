...
class _MountainRouteScreenState extends State<MountainRouteScreen> {
  ...
  bool _shouldHideMap = false; // 검색 중 맵을 숨기기 위한 변수 추가
  ...
  void _setupSearchListener() {
    _searchFocusNode.addListener(() {
      if (_searchFocusNode.hasFocus) {
        // 포커스 얻으면 검색 상태로 변경하고 맵 숨기기
        setState(() {
          _isSearching = true;
          _shouldHideMap = true; // 맵 숨기기
        ...
        // 포커스 잃으면 오버레이 제거하고 맵 다시 표시
        _removeOverlay();
        Future.delayed(Duration(milliseconds: 300), () {
          if (mounted) {
            setState(() {
              _shouldHideMap = false;
            });
          }
        });
      }
                    ...
                    child: CompositedTransformTarget(
                      ...
                      child: Container(
                        ...
                        child: TextField(
                          ...
                          textAlignVertical:
                              TextAlignVertical.center, // 추가: 수직 정렬 중앙으로
                              ...
                              return Container(
                                ...
                                decoration: BoxDecoration(
                                  ...
                                  boxShadow: [
                                    BoxShadow(
                                      color: isSelected
                                          ? Color(0xFF52A486).withOpacity(
                                              0.2) // 선택됐을 때는 앱 메인 색상으로
                                  ...
                                  border: Border.all(
                                    color: isSelected
                                        ? Color(
                                            0xFF52A486) // 선택됐을 때 테두리 색상도 앱 메인 색상으로
                                        : Colors.transparent,
                                    width: 2,
                                  ),
                                ),
...
// 등산로 정보 아이템 위젯
  ...
  Widget _buildMapContainer() {
    return Container(
      ...
      child: _routes.isEmpty || _shouldHideMap // 검색 중이거나 경로가 없을 때는 맵 대신 메시지 표시
          ? Center(
              child: Column(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  ...
                  SizedBox(height: 12),
                  Text(
                    _shouldHideMap ? '검색 중...' : '등산로 정보가 없어요',
                    style: TextStyle(
                      fontSize: 15,
                      color: Colors.grey[600],
                    ),
  ...
  void dispose() {
    if (_mapController != null) {
      try {
        ...
        _mapController = null;
        debugPrint('맵 컨트롤러 리소스 해제 완료');
      } catch (e) {
        debugPrint('맵 컨트롤러 리소스 해제 중 오류: $e');
      }
    }
    super.dispose();
  }
